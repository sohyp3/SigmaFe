...
const SignIn = () => {
  ...
  useEffect(() => {
    const isAuthenticated = authState?.authenticated ?? null;
    setIsLoggedIn(isAuthenticated);
  }, [authState?.authenticated]);
  
  return (

    <View className="flex-1 p-4 mt-2 text-3xl">
      <Text className="text-5xl font-bold">Sign In</Text>
      <Text>helo: {isLoggedIn}</Text>
      <Link href="/(root)/(tabs)/home">Go To Home?</Link>
